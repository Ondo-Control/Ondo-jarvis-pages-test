(() => {
  ...
  async function executeHashCommand() {
    ...
    try {
      ...
      setMarker('error', 'ONDO Alexa Visual Control · ungültiger Befehl', {
        ok: false,
    ...
    setMarker('running', `ONDO Alexa Visual Control · läuft · ${clip(command.command_id, 48)}`, {
    ...
    try {
      ...
      if (response?.ok) {
        setMarker(
          'ready',
          `ONDO Alexa Visual Control · fertig · ${response.total_ms ?? '?'} ms`,
          response
        );
      } else {
        setMarker(
          'error',
          `ONDO Alexa Visual Control · Fehler · ${clip(response?.error || 'UNKNOWN', 80)}${response?.detail ? ` · ${clip(response.detail, 180)}` : ''}`,
          response || { ok: false, error: 'NO_RESPONSE' }
        );
      }
    } catch (error) {
      setMarker('error', `ONDO Alexa Visual Control · Fehler · ${clip(error?.message || error, 80)}`, {
        command_id: command.command_id,
        ok: false,
  ...
  setMarker('ready', `ONDO Alexa Visual Control aktiv · v${VERSION}`);
  executeHashCommand();
